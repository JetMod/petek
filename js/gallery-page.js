...
/* Hero-коллаж: по клику три фото циклически меняют позиции (слоты) */
(function () {
  var stack = document.getElementById('gallery-hero-stack');
  if (!stack) return;

  var imgs = stack.querySelectorAll('.gallery-hero__shot');
  if (imgs.length !== 3) return;

  var SLOT_CLASSES = ['gallery-hero__shot--slot1', 'gallery-hero__shot--slot2', 'gallery-hero__shot--slot3'];

  /* slotContent[s] — индекс картинки (0..2), которая стоит в визуальном слоте s */
  var slotContent = [0, 1, 2];

  function applySlots() {
    slotContent.forEach(function (imgIdx, slotIdx) {
      var el = imgs[imgIdx];
      SLOT_CLASSES.forEach(function (c) {
        el.classList.remove(c);
      });
      el.classList.add(SLOT_CLASSES[slotIdx]);
    });
  }

  function rotateSlots() {
    slotContent = [slotContent[2], slotContent[0], slotContent[1]];
    applySlots();
  }
